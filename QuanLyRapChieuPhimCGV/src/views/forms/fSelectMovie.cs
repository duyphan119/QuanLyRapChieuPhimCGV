...
{
    public partial class fSelectMovie : Form
    {
        ...
        public fSelectMovie(ucBookTicket uc)
        {
            preComponent = uc;
            InitializeComponent();

            dao_m.getAllCanWatch(dtDate.Value).ForEach(movie =>
            {
                if(movies.FindIndex(m=>m.id == movie.id) == -1)
                {
                    movies.Add(movie);
                    cards.Add(new ucCardMovie(this, movie, dtDate.Value));
        ...
        public void getTime(Movie movie, string time)
        {
            ...
            DateTime dt = new DateTime(dtDate.Value.Year, dtDate.Value.Month, dtDate.Value.Day, hour, minute, 0);
